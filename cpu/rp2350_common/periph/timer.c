 ...
 *
 * The RP2350 features two 64 bit timers counting 1 us ticks generated from
 * clk_ref by the TICKS block. Each timer has four alarms that match on the
 * lower 32 bits of the counter, each alarm has its own interrupt line.
 *
...
static void _timer_reset(tim_t dev)
{
     ...
     * counter once TIMEHW is written */
    DEV(dev)->TIMELW = 0;
    DEV(dev)->TIMEHW = 0;
    irq_restore(state);
}
...
{
    /* The latched interrupt flags are write-1-clear */
    ...
    if (flag_periodic[dev] & (1u << channel)) {
        ...
        /* Re-arm the alarm by writing its current value again */
...
int timer_init(tim_t dev, uint32_t freq, timer_cb_t cb, void *arg)
{
    ...
    /* The TICKS block divides clk_ref down to 1 MHz (us ticks), other
    ...
    /* Generate one tick per microsecond from clk_ref, which runs from the
     * crystal oscillator */
    if (DEV(dev) == TIMER0) {
...
int timer_set(tim_t dev, int channel, unsigned int timeout)
{
    ...
    if (!timeout) {
        /* Execute the callback immediately if the timeout is 0, to catch
         * the case that a tick happens right before arming the alarm and
         * causes a full timer period to elapse */
        if (ctx_cb[dev]) {
            ctx_cb[dev](ctx_arg[dev], channel);
        }
    }
        ...
        /* An alarm matches on the lower 32 bits of the 64 bit counter */
...
int timer_clear(tim_t dev, int channel)
{
    ...
    unsigned state = irq_disable();
    /* The ARMED bits are write-1-clear */
...
/* Overwrites the WEAK_DEFAULT alarm ISRs of timer_config[0] (TIMER0) */

...
int uart_mode(uart_t uart, uart_data_bits_t data_bits,
              uart_parity_t uart_parity, uart_stop_bits_t stop_bits) {
  assert((unsigned)uart < UART_NUMOF);
  UART0_Type *dev = uart_config[uart].dev;

  io_reg_atomic_clear(&dev->UARTCR, UART0_UARTCR_UARTEN_Msk |
                                        UART0_UARTCR_TXE_Msk |
                                        UART0_UARTCR_RXE_Msk);

  /* Beware of strange hardware bug: If the configuration bitmask is prepared in
   * register and transferred with a single 32 bit write (updating both parity
   * and number of data bits at the same time), the configuration change of the
   * parity bits will not take place until after the next char send out. If the
   * configuration is updated in multiple bus accesses, it will apply directly
   * to the next char. So: Double check e.g. with tests/periph/uart_mode after
   * touching the initialization code here */
  ...
  switch (uart_parity) {
    ...
    case UART_PARITY_EVEN:
      io_reg_atomic_set(&dev->UARTLCR_H,
                        UART0_UARTLCR_H_EPS_Msk | UART0_UARTLCR_H_PEN_Msk);
      break;
  ...
  io_reg_atomic_set(&dev->UARTCR, UART0_UARTCR_UARTEN_Msk |
                                      UART0_UARTCR_TXE_Msk |
                                      UART0_UARTCR_RXE_Msk);
...
void uart_init_pins(uart_t uart) {
  assert((unsigned)uart < UART_NUMOF);
  /* not reusing gpio_init() here to not send a char of garbage upon init. Also,
   * we can use a lower drive strength here, hopefully limiting the current a
   * bit when TX is connected to the recipients TX by accident. */
...
static void _poweron(uart_t uart) {
  uint32_t reset_bit_mask =
      (uart) ? RESETS_RESET_uart1_Msk : RESETS_RESET_uart0_Msk;
...
int uart_init(uart_t uart, uint32_t baud, uart_rx_cb_t rx_cb, void *arg) {
  ...
  /* Beware: Changes to the symbol rate only take affect after touching the
   * UARTLCR_H register, which is done in uart_mode(). */
...
void isr_handler(uint8_t num) {
  ...
  if (status & UART0_UARTMIS_RXMIS_Msk) {
    ...
    if (data &
        (UART0_UARTDR_BE_Msk | UART0_UARTDR_PE_Msk | UART0_UARTDR_FE_Msk)) {
